...
var comparisonRefName = "REF_MASTER_CHIKV";
...
_.each(_.keys(tipAlignments), function(alignmentName) {
	...
	_.each(featuresList, function(featureObj) {
		...
		if(featureObj.name == "non_structural_polyprotein") {
			return; 
		}
		if(featureObj.name == "structural_polyprotein") {
			return; 
		}
	...
	_.each(_.values(replacementsSet), function(replacementObj) {
		...
		var variationName = "chikv_aa_rpl:"+replacementObj.id;
		var variationExists = false;
	
		glue.inMode("reference/REF_MASTER_CHIKV/feature-location/"+replacementObj.feature, function() {
		...
		if(!variationExists) {
			...
			if(replacementObj.refAa != '*' && replacementObj.refAa != 'X'
				...
				glue.inMode("module/chikvHanada2006ReplacementClassifier", function() {
				...
				glue.inMode("module/chikvGrantham1974DistanceCalculator", function() {
				...
				glue.inMode("module/chikvMiyata1979DistanceCalculator", function() {
					miyataDistance = glue.command(["distance", replacementObj.refAa, replacementObj.replacementAa]).miyata1979DistanceResult.distance;
				});
			}

			
			glue.command(["create", "custom-table-row", "chikv_replacement", replacementObj.id]);
			glue.inMode("custom-table-row/chikv_replacement/"+replacementObj.id, function() {
				...
				glue.command(["set", "link-target", "variation", 
					"reference/REF_MASTER_CHIKV/feature-location/"+replacementObj.feature+
		...
		_.each(replacementObj.memberSeqs, function(memberObj) {
			...
			glue.command(["create", "custom-table-row", "chikv_replacement_sequence", linkObjId]);
		
			glue.inMode("custom-table-row/chikv_replacement_sequence/"+linkObjId, function() {
				glue.command(["set", "link-target", "chikv_replacement", "custom-table-row/chikv_replacement/"+replacementObj.id]);
